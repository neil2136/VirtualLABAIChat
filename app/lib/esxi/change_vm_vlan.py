...
def ChangeVMVlan(host, user, pwd, vmname, adapter, vlan, nicstatus):
    content = LoginEsxi(host, user, pwd)
    datacenters = content.rootFolder.childEntity
    # vmFolders = content.rootFolder.childEntity[0].vmFolder.childEntity
    print(f'host: {host}, user: {user}, pwd: {pwd}, vmname: {vmname}, adapter: {adapter}, vlan: {vlan}, nicstatus: {nicstatus}')
    try:
        device_change = []
